...
def load_data(data_dir):
    ...
    for i in range(NUM_CATEGORIES):
        #get the path for the current subdirectory
        subdirname =os.path.join(data_dir,str(i))
            ...
            for filename in os.listdir(subdirname):
                ...
                if os.path.isfile(filepath):
                    ...
                    if image is not None:
                        #resize the image to be the same size as all other images
                        res_image=cv2.resize(image,(IMG_WIDTH,IMG_HEIGHT))
